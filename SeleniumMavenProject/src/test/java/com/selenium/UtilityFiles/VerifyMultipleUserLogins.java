...
//count total columns from excel
		...
		for (int i = 0; i < s.getRows(); i++) 
		{
				...
				for (int j = 0; j < s.getColumns(); j++) 
				{
					...
					ws.addCell(new Label(j, i, 	a[i][j] ));
...
try{
	...
	//String actualvalue=getObject("Invalid_login").getText();
	// to verify whether the particular text is present or not on the page
...
FileUtils.copyFile(scrFile, new File(config.getProperty("ScreenshotPath")+".png"));	
